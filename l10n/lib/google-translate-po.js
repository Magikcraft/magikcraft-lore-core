...
function main({ poFile, toLang, outputFile }, cb) {
  ...
  return new Promise((resolve, reject) => {
	  po.load(poFile, po => translate(po.items, toLang)
...
function translate(items, toLang, fromLang = "en") {
  return new Promise((resolve, reject) => {
    ...
    function doTranslate(item, _cb) {
      ...
      if (length && toTranslate[0] === "`" && toTranslate[length - 1] === "`" && !toTranslate.split['`']) {
        return _cb(null, {
...
module.exports = ({poDir}) => (toLang, cb) => googleTranslatePO({poDir, toLang, cb});
